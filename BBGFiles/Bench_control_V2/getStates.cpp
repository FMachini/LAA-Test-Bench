...
void getData::encoderInitialPosition(double &eH0, double &eV0)
{
...
std::vector<double> getData::encoderPosition(const double &HP0,const double &VP0,double &eptrV,double &eptrH)
{
	    ...
	    // eVertical_p1 = eqepVertical.get_position();
            eVertical_Ang = ((eqepVertical.get_position()-HP0)*360/4000);
            eVertical_Ang  *= -1;             
	    ...
	    // eHorizontal_p1= eqepHorizontal.get_position();
            eHorizontal_Ang= ((eqepHorizontal.get_position()-VP0)*360/4000);
	    eHorizontal_Ang *= -1;            
...
// Rearrange states vector to x = [theta_1,theta_2,theta_4] (already filtered!)
void getData::getStateVector(std::vector<double> fromImu, std::vector<double> fromEncoder, arma::mat &SV,int numit)
{
    SV(numit,0) = fromEncoder[0]*DEG2RAD;
    SV(numit,1) = fromEncoder[1]*DEG2RAD;    
    SV(numit,2) = fromImu[0]*DEG2RAD;
    SV(numit,3) = fromEncoder[2]*DEG2RAD;
    SV(numit,4) = fromEncoder[3]*DEG2RAD;
    SV(numit,5) = fromImu[1]*DEG2RAD;

//    std::vector<double> v;
//    
//    //v.push_back(fromImu[1]);
//    v.push_back(fromEncoder[0]);
//    v.push_back(fromEncoder[1]);
//    v.push_back(fromImu[0]);
//    //v.push_back(fromImu[1]);
//    v.push_back(fromEncoder[2]);
//    v.push_back(fromEncoder[3]); 
//    v.push_back(fromImu[2]);
//    
//    return v;
}
