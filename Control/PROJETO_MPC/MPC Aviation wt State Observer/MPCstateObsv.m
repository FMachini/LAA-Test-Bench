...
Cd = eye(6,7);
Cx1 = zeros(6,3);Cx1(3,1) = 1;Cx1(4,2)=1;Cx1(5,3) = 1;
Cx = [Cd Cx1];
...
L = L';
damp(Ad-L*Cd)
Kobs = place(Ad',Cd',[0.89 0.9 0.701 0.702 0.703 0.699 0.698]);
Kobs = Kobs';
...
for k = 2:kend
   tic;
   yout =  Cd *(xp(:,k) + Cd'*0*pi/180 * randn(3,1));
   %yout = Cd*xpert;
   tt(k+1) = tt(k)+Ts;
   xlqrk = xp(1:nxlqr,k);
   %xmpck = xp(1:end,k);
   ...
   ye = Cd*xmpck(:,k-1);
   xmpck(:,k) = Ad*xmpck(:,k-1) + L*(yout - ye) + Bd*up(:,k);
   
   
   %% LQR control law
   ulqr(:,k) = -Klqr*xlqrk;
   %xmpc = xp(1:end,k)+3*pi/180 * rand(2,1);
   %% MPC
