...
Control::Control(int numStates, int numInput, int numTrack, arma::mat refVal, double timeSample){
        ...
        std::cerr<< "Reference set to: Theta_2 "<< ref(0,0) << " and Theta_4 "<< ref(1,0)<< std::endl;
        dt = timeSample;
...
arma::mat Control::computeOutput(arma::mat States, int cont){
    ...
    Aux(0,0)= States(cont,1);
    Aux(1,0) = States(cont,2);    
    Aux(2,0)= States(cont,4);
    Aux(3,0) = States(cont,5);
    
    Aux2(0,0)= States(cont,1);
    Aux2(1,0) = States(cont,2); 
