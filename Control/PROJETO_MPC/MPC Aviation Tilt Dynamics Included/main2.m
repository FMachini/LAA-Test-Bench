...
for k = 2:kend
   ...
   tt(k+1) = tt(k)+Ts;
   xlqrk = xp(1:nxlqr,k);
   %filteredState(:,k) = mediaMovel(xp,k,5);
   
   %xmpck = xp(1:end,k);
   yout = [xp(1,k);xp(3,k);xp(5,k)];
   ...
   %% Updating system state
    ...
    %            umpc(1:2,k)];
     up(:,k) =  [ulqr(:,k) + umpc(end,k)*ones(2,1);
                 umpc(1:2,k)];
   %discrete
    xp(:,k+1) = Ad6*xp(:,k) + Bd6*up(:,k);
    
    
end

%% Exporting data
%save 'wthTiltDynamics'
save 'noTiltDynamics'
