...
std::vector<double> Sensors::encoderPosition()
{
	...
	toc1 = double(clock() - tic1)/CLOCKS_PER_SEC;
	eVertical_Ang = ((eqepVertical.get_position()-eV0)*360/4000);
	eVertical_Ang  *= -1;
	eVertical_Vel = (eVertical_Ang  - eptrV)/toc1;
	eptrV = eVertical_Ang;
	tic1 = clock();


	eHorizontal_Ang= ((eqepHorizontal.get_position()-eH0)*360/4000);
	toc2 = double(clock() - tic2)/CLOCKS_PER_SEC;
	eHorizontal_Ang *= -1;
	eHorizontal_Vel = (eHorizontal_Ang - eptrH)/toc2;
...
//		SV(numit,1) = fromEncoder[2]*DEG2RAD; //Theta_1 Reverse direction
//		SV(numit,0) = -fromEncoder[0]*DEG2RAD;  //Theta_2
//		SV(numit,2) = fromImu[0]*DEG2RAD;      //Theta_4
//		SV(numit,4) = fromEncoder[3]*DEG2RAD; //Theta_1_dot Reverse direction
//		SV(numit,3) = -fromEncoder[1]*DEG2RAD;  //Theta_2_dot
//		SV(numit,5) = fromImu[1]*DEG2RAD;      //Theta_4_dot
