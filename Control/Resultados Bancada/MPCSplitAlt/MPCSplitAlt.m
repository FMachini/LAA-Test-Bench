...
[Aexd,Bexd,Cexd] = expandTiltDynamics2(Ac,Bc,Ts);
%Qob = diag([1e-2 1e-2 20 10 20 10 1e-3 1e-3 1e-3 1e-3]);
Qob = diag([1e-2 1e-2 20 10 20 10 1e3 1e3 1e3 1e3]);
Rob = diag([5 10 1 1 1 1]);


ndist = 0; %number of disturbances in the model
Aobs = [Aexd zeros(10,ndist);
        zeros(ndist,10) eye(ndist)];
Bobs = [Bexd; zeros(ndist,4)];
Cx1 = zeros(6,ndist); Cx1(1,1)=1;Cx1(2,2)=1;
Cobs = [Cexd Cx1];



 Lobs = dlqr(Aexd',Cexd', Qob,Rob);
% % L = inv(Ax)*L;
Lobs = Lobs';
%damp(Aobs-Lobs*Cobs)
...
dlmwrite('Matrices/Kp.txt',Kp);
dlmwrite('Matrices/Kx.txt',Kx);
dlmwrite('Matrices/Lobs.txt',Lobs);
dlmwrite('Matrices/Ad.txt',Aexd);
dlmwrite('Matrices/Bd.txt',Bexd);
dlmwrite('Matrices/Cd.txt',Cexd);
dlmwrite('Matrices/MPCParameters.txt',[N M nxp nup nympc nxlqr nulqr numpc ndist Ts]);
% load Mats
% M =6;
...
int_e = 0;
CcMpc = zeros(8,10);
CcMpc(1,1)=1;CcMpc(2,2)=1;CcMpc(3,3)=1;CcMpc(4,4)=1;CcMpc(5,7)=1;
CcMpc(6,8)=1;CcMpc(7,9)=1;CcMpc(8,10)=1;
for k = 2:kend
   tic;
   %xp(:,k) = xp(:,2);
   yout = xp(1:6,k);
   tt(k+1) = tt(k)+Ts;
   xlqrk = xp(1:nxlqr,k)+0*3*pi/180.*randn(2,1);
   
   %xmpck(:,k)  = xp(1:end,k);
   
   % State Observer prediction
   ye = xmpck(1:6,k-1);
   xmpck(:,k) = Aexd*xmpck(:,k-1) + Lobs*(yout - ye) + Bexd*up(:,k-1);
   %ye = Cd*xmpck(:,k+1);
   ...
   %% Altitude Control Law
    e = AltRef - xmpck(5,k);
    int_e = int_e + e*Ts;
    ualt(:,k) = Kp*int_e - Kx*xmpck(5:6,k);
    
   %% MPC
   %fmpc = Phi*[eye(6,9)*xmpck(:,k+1); umpc(:,k-1)];
   fmpc = Phi*[CcMpc*xmpck(:,k); umpc(:,k-1)];
