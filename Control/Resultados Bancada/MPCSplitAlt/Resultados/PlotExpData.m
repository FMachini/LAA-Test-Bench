...
for i = 1:2
    ...
    arq = {'MPCSplitAltV6.txt','ResultsTeste1_28_01_21.txt'};
    leg = {'MPC','LQT'};
    %leg = {'Exp 1','Exp 2','Exp 3'};
    A = textread(arq{i});
    if i == 2 
        A(:,1:6) = A(:,1:6)*180/pi;
        A(:,9:10) = A(:,9:10)*180/pi;
        utrim = 0;
        zZero = 0;
    end
    A(:,1:6) = A(:,1:6)*1;
...
subplot(1,2,1)
hold on
plot([2:kend].*Ts, A(2:end,7)+utrim, 'LineWidth',2)
...
subplot(1,2,2)
hold on
plot([2:kend].*Ts, A(2:end,8)+utrim, 'LineWidth',2)
