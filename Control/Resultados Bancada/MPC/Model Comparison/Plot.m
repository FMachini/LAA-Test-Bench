...
leg = {'Linear model','Theta 1 dynamics','Tilt Dynamics Included','Experimental Data'};
Ts =0.02;
for i = 1:4

    if i ==4
    arq = {'Exp1_26_03_21.txt'};
    
    A = textread(arq{1});
    kend = size(A,1)-1;
    A = A';
    xp = zeros(6,size(A,2));
    xp(1,1:end) = A(3,:)*pi/180;
    xp(2,1:end) = A(6,:);
    xp(3,1:end) = A(1,:)*pi/180;
    xp(4,1:end) = A(4,:);
    xp(5,1:end) = A(2,:)*pi/180;
    xp(6,1:end) = A(5,:);
    up = zeros(4,size(A,2)-1);
    up(1:2,:) = A(7:8,1:end-1)-1.7;
    up(1:2,1) = up(1:2,2);
    up(3:end,:) = A(9:10,1:end-1)*pi/180;
    else
    arq = {'noTiltDynamics.mat', 'Theta1Dynamics.mat','wthTiltDynamics.mat'};
    %leg = 'Constrained';
    load(arq{i});
    kend = size(xp,2)-1;
    end
