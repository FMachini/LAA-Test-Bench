...
Qob = diag([1e-3 1e-3 50 10 50 10 1e-4 1e-4 1e-4 1e-4 1 1]);
Rob = diag(100*[10 10 0.1 1 0.1 1]);

numDis = 2;
Ax = [Ad zeros(10,numDis);
        zeros(numDis,10) eye(numDis)];
 Bx = [Bd; zeros(numDis,4)];
 Cx1 = zeros(6,numDis); Cx1(3,1)=1;Cx1(5,2)=1;
 Cx = [Cd Cx1];



Lx = dlqr(Ax',Cx', Qob,Rob);
% L = inv(Ax)*L;
Lx = Lx';
damp(Ax-Lx*Cx)
% Lx = [L; eye(3)];
...
for k = 2:kend
   ...
   ye = Cd*xmpck(:,k-1);
   xmpck(:,k) = Ad*xmpck(:,k-1) + L*(yout - ye) + Bd*up(:,k-1);
   ...
   fmpc = Phi*[xmpck(1:10,k); umpc(:,k-1)];
